...
# While you're here, make sure you group name is filled in above.
...
List all group members here (maximum *3* and minimum *2*):

- Tran Tram Anh (Sophia) Thai
- Kaya Darling
...
at_cyclone <- str_c(cyclone_data_address, at_cyclone_filename, sep = "") |>
    ...
    separate_wider_position(
        BasinNumberYear,
        # Specify the widths
        widths = c(Basin = 4, Number = 2, NameYear = 2)
    ) |>
    separate_wider_position(
        `1`,
        # Specify the widths
...
Now, bind the two data frames together (row-wise). Since the `Basin` column already contains the information about which basin the cyclone was in, there's no need to specify an ID. (Hint: don't over think this one.)
...
convert_latlon <- function(latlon) {
    # YOUR CODE HERE
  if_else(
    str_detect(latlon, "[NS]$"),  # check if N/S for latitude
    parse_number(latlon) * if_else(str_detect(latlon, "N$"), 1, -1),
    parse_number(latlon) * if_else(str_detect(latlon, "E$"), 1, -1)
  )
}
...
Perhaps there's some relationship between governments' response to the pandemic and governments' response to climate change? The following data will load in some information about COVID-19 in countries around the world.

```{r}
#owid_address <- "https://covid.ourworldindata.org/data/owid-covid-data.csv"

owid_address <- "https://raw.githubusercontent.com/owid/covid-19-data/refs/heads/master/public/data/owid-covid-data.csv"
...
That's slightly less than what we need to deal with! Overall, the `pop_data` has more countries in it than `aware_fixed_country`, so we'll just do our best to change the countries in `aware_fixed_country` to match `pop_data`. Note that there a lot of [controversial politics](https://www.youtube.com/watch?v=3nB688xBYdY) involved in defining a "country" (esp. Taiwan, Hong Kong, West Bank and Gaza, etc.), and I'll avoid these as much as possible for now.
...
setdiff(aware_countries, pop_countries) |> sort() |> length()
...
aware_pop <- aware_fixed_country_match |>
  inner_join(
    pop_data |> 
      select(country, total_population = SP.POP.TOTL),
    by = "country"
  ) |>
  select(
    country,
...
covid_2020_match_country <- covid_2020_fixed |>
  semi_join(aware_pop, by = "country")
covid_2020_match_country

nrow(covid_2020_match_country)
...
aware_covid |>
    ...
    ggplot() +
    aes(x = value, y = covid_per_capita, colour = continent) +  #it supposed to be covid_per_capita but it first it was cases_per capita
    geom_point() +
...
For the most part, the "Ladder score" column is the most important - this is the WHR's main question that gets at how "happy" the people of a country are. You may choose to explore other options though - perhaps people's perceptions of corruption affect their response to climate issues?
...
- Remove any rows for which `ladder_score` is `NA`.
...
happiness_raw <- read.xlsx("DataForTable2.1.xlsx") |>
    clean_names()

# Check the years available
happiness_raw |> 
...
# The correct approach - ensure we're getting the true most recent year
happiness <- happiness_raw |>
    rename(country = country_name) |>
    filter(!is.na(ladder_score)) |>
    # Group by country and get the row with the maximum year
    group_by(country) |>
    filter(year == max(year)) |>
    # If there are ties (same max year), take the first one
    slice(1) |>
    ungroup()

# Check some sample countries and their years
happiness |> 
    filter(country %in% c("Finland", "Denmark", "United States", "Afghanistan")) |>
    select(country, year)
